...
bool readTimeMessage()
{
    ...
    // M5Stick C Plus. Transmitter day start
      ...
      M5.Lcd.printf("Dex.day: %d", currentTime / (60*60*24)); 
      last_day = currentTime;
    // ---
...
bool readBatteryStatus()
{
    ...
    // M5Stick C Plus. Battery state
      uint16_t battery_vol = (uint16_t)(batteryStatusRxMessage[2] + batteryStatusRxMessage[3]*0x100);
      ...
      M5.Lcd.printf("Dex.bat: %d", battery_vol); 
      
      M5.Lcd.setTextColor(GREEN);
      last_bat = battery_vol;
    // ---
...
bool readGlucose()
{
    ...
    // M5Stick C Plus. Glicose level
        ...
        current_glucose/=18.0;
        
        if (last_glucose == 0.0)
          last_glucose = current_glucose;
        
        drawTrend(last_glucose, current_glucose);
        M5.Lcd.printf("%4.1f", current_glucose); 
