...
static std::string transmitterID = "40MEYD";              /* Set here your transmitter ID */                            // This transmitter ID is used to identify our transmitter if multiple dexcom transmitters are found.
...
RTC_SLOW_ATTR static float last_glucose = 0.0;
RTC_SLOW_ATTR static float pre_last_glucose = 0.0;
RTC_SLOW_ATTR static uint16_t last_bat = 0;
RTC_SLOW_ATTR static uint32_t last_day = 0;
static float current_glucose = 0.0;
...
class MyClientCallback : public BLEClientCallbacks
{
    ...
    void sleepHibernation()
    {
        ...
        esp_sleep_enable_timer_wakeup(270 * 1000000);
...
void setup() 
{
    ...
    if (last_glucose == 0.0) {
      ...
      M5.Lcd.print("Connecting to transmitter..."); 
      vbat_l();
    } else {
    // M5Stick C Plus. Glicose level
        M5.Lcd.setTextSize(5);
        M5.Lcd.setCursor(8, 50);
        
        drawTrend(last_glucose, pre_last_glucose);
        M5.Lcd.printf("%4.1f", last_glucose); 

        pre_last_glucose = last_glucose;    
    // ---      
    // M5Stick C Plus. Transmitter day start
      M5.Lcd.setTextSize(1.2);
      M5.Lcd.setCursor(170, 3);
      M5.Lcd.printf("Dex.day: %d", last_day / (60*60*24)); 
    // ---
    // M5Stick C Plus. Battery state
      if (last_bat < 300)
        M5.Lcd.setTextColor(RED);
      
      M5.Lcd.setTextSize(1.2);
      M5.Lcd.setCursor(3, 3);
      M5.Lcd.printf("Dex.bat: %d", last_bat); 
      M5.Lcd.setTextColor(GREEN);
...
void drawTrend(float old_result, float new_result) {
  uint16_t greenColor = TFT_GREEN;
  uint16_t blueColor = TFT_BLUE;
  uint16_t redColor = TFT_RED;

  uint16_t glColor = greenColor;
  
  int arrowAngle = 0;
  if((old_result - new_result) >= 1.0) {
    arrowAngle = 90;
    glColor = blueColor;
  }
  else 
    if((old_result - new_result) >= 0.8)
      arrowAngle = 75;
    else 
        if((old_result - new_result) >= 0.5)
          arrowAngle = 45;
        else 
            if((old_result - new_result) <= 0.3 && (old_result - new_result) >= 0.0)
              arrowAngle = 0;
            else 
                if((old_result - new_result) <= -0.5)
                  arrowAngle = -45;
  ...
  drawArrow(180, 70, 14, arrowAngle+85, 27, 42, glColor);                          
}
